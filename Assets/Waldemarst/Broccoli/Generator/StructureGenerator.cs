...
namespace Broccoli.Generator
{
	...
	public class StructureGenerator {
		...
		/// <param name="structureLevel">StructureLevel instance to generate structures from.</param>
		/// <param name="parentStructure">Structure that will receive the structures.</param>
		/// <param name="structureIndex">Structure that will receive the structures.</param>
		public delegate void OnBeforeGenerateStructures (List<Structure> structures, StructureLevel structureLevel, Structure parentStructure, int structureIndex);
		/// <summary>
		/// Called after a StructureLevel has generated structures for a structure.
		/// </summary>
		/// <param name="structureLevel">StructureLevel instance to generate structures from.</param>
		...
		List<Structure> GenerateCustomTrunkStructures (TrunkStructureLevel trunkStructureLevel, List<BezierCurve> trunkCurves, List<Structure> originalStructures)
		{
			...
			for (int i = 0; i < trunks.Count; i++) {
				...
				if (trunkStructureExists) {
					originalStructures [i].branch.id = originalStructures [i].id;
					originalStructures [i].branch.curve = trunks[i].curve;
		...
		/// <param name="structureIndex">Index of the structure on their parent structure.</param>
		...
		{
			// OnBefore delegate.
			onBeforeGenerateStructures?.Invoke (structures, structureLevel, parentStructure, structureIndex);
			...
			for (int i = 0; i < mergedStructures.Count; i++) {
				if (structureLevel.structureLevels != null) {
					for (int j = 0; j < structureLevel.structureLevels.Count; j++) {
						GenerateStructuresRecursive (mergedStructures[i].childrenStructures, structureLevel.structureLevels[j], mergedStructures[i], mergedStructures[i].id);
					}
				}
			}
