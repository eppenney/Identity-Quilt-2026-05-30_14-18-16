...
namespace Broccoli.Utils
{
    public class WindJob {
        ...
        private List<Vector4> uv0s = new List<Vector4> ();
		...
		struct WindJobImpl : IJobParallelFor {
			...
			public void Execute (int i) {
				...
				ApplyUV0 (vertexStart, vertexEnd, swingPhase, spOrigin);
				ApplyUV1 (vertexStart, vertexEnd, xPhase, yPhase);
				ApplyUV2 (vertexStart, vertexEnd, spOrigin);
			}
			public void ApplyUV0 (int vertexStart, int vertexEnd, float swingPhase, Vector3 origin) {
				for (int i = vertexStart; i < vertexEnd; i++) {
					uv0s [i] = new Vector4 (uv0s [i].x, uv0s [i].y, Vector3.Distance (origin, vertices [i]) * windAmplitude, swingPhase);
					/// x: mapping U component.
					/// y: mapping V component.
			...
			public void ApplyUV1 (int vertexStart, int vertexEnd, float xPhase, float yPhase) {
				//float xPhase = Random.Range (0f, 5f);
				//float yPhase = Random.Range (0f, 14f);
				for (int i = vertexStart; i < vertexEnd; i++) {
					uv1s [i] = new Vector4 (uv0s [i].z, uv0s [i].w, xPhase, yPhase);
					/// z: Adds wind data: x phased values (0-5).
					/// w: Adds wind data: y phased values (0-14).
				}
			}
			public void ApplyUV2 (int vertexStart, int vertexEnd, Vector3 origin) {
				for (int i = vertexStart; i < vertexEnd; i++) {
					uv2s [i] = new Vector4 (vertices [i].x, vertices [i].y, vertices [i].z, vertices [i].z - origin.z);
					/// x: vertex x position.
					/// y: vertex y position.
					/// z: vertex z position..
					/// w: vertex z position on the branch origin.
        ...
        public void ExecuteJob () {
			...
			WindJobImpl _meshJob = new WindJobImpl () {
				...
				uv0s = new NativeArray<Vector4> (uv0s.ToArray (), Allocator.TempJob),
                uv1s = new NativeArray<Vector4> (uv0s.Count, Allocator.TempJob),
                uv2s = new NativeArray<Vector4> (uv0s.Count, Allocator.TempJob),
			};
			// Execute the job .
            ...
            targetMesh.SetUVs (0, _meshJob.uv0s);
            targetMesh.SetUVs (1, _meshJob.uv1s);
            targetMesh.SetUVs (2, _meshJob.uv2s);
            ...
            _meshJob.uv0s.Dispose ();
            _meshJob.uv1s.Dispose ();
