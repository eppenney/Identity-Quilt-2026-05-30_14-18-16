...
namespace Broccoli.BroccoEditor
{
    public class SproutLabMappingPanel {
		...
		private static string sproutTintModeDistributionName = "sprout-a-tint-mode-distribution";
		...
		private CurveField sproutTintModeDistributionElem;
		...
		void InitializeSproutStyle () {
			...
			// Tint Distribution.
			sproutTintModeDistributionElem = container.Q<CurveField> (sproutTintModeDistributionName);
			sproutTintModeDistributionElem?.RegisterValueChangedCallback(evt => {
				if (listenGUIEvents) {
					OnBeforeEdit ();
					sproutLabEditor.selectedSproutStyle.sproutTintDistribution = evt.newValue;
					OnEdit (false, false, true, false, true);
				}
			});
		...
		void ResetSproutTint (ClickEvent evt) {
			if (listenGUIEvents) {
				...
				sproutLabEditor.selectedSproutStyle.sproutTintDistribution = AnimationCurve.Linear (0f, 0f, 1f, 1f);
