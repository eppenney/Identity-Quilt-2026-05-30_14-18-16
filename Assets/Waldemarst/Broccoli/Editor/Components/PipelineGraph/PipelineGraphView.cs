...
namespace Broccoli.TreeNodeEditor
{
    public class PipelineGraphView : GraphView {
        ...
        public void Init (Vector2 offset, float zoom) {
			...
			currentZoom = zoom;
			contentViewContainer.style.scale = new Scale(new Vector3 (zoom, zoom, zoom));

			// Offset
			currentOffset = offset;
			contentViewContainer.style.translate = new Translate(offset.x, offset.y);
        ...
        public bool LoadPipeline (Pipeline pipelineToLoad, Vector2 offset, float zoom = 1f) {
            if (pipelineToLoad != null) {
				...
				this.SetupZoom (0.05f, ContentZoomer.DefaultMaxScale, 0.05f, zoom);
				currentZoom = zoom;
				this.contentViewContainer.style.scale = new Scale(new Vector3 (zoom, zoom, zoom));
				// Offset.
				currentOffset = offset;
				this.contentViewContainer.style.translate = new Translate(offset.x, offset.y);
        ...
        private void _ViewTransformChanged (GraphView graphView) {
			...
			// If pan done.
			if (this.contentViewContainer.resolvedStyle.translate != currentOffset) {
				onPanDone?.Invoke (this.contentViewContainer.resolvedStyle.translate, currentOffset);
				currentOffset = this.contentViewContainer.resolvedStyle.translate;
			}
        }
