...
namespace Broccoli.TreeNodeEditor
{
    ...
    */
    ...
    public class StructureEdgeDragHelper<TEdge> : EdgeDragHelper where TEdge : Edge, new()
    {
        ...
        public override void Reset(bool didConnect = false)
        {
            ...
            if (m_WasPanned)
            {
                if (!resetPositionOnPan || didConnect)
                {
                    ...
                    Scale scale = m_GraphView.contentViewContainer.resolvedStyle.scale;
                    Vector3 s = scale.value;
        ...
        {
            Vector3 diff = m_GraphView.resolvedStyle.translate - m_PanDiff;
            m_GraphView.style.translate = new Translate(diff.x, diff.y);
            m_WasPanned = true;
        }
        ...
        public override bool HandlePointerDown(PointerDownEvent evt) { return true;  }
