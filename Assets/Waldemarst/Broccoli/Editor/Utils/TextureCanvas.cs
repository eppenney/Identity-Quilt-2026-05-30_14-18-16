...
namespace Broccoli.TreeNodeEditor
{
    ...
    public class TextureCanvas : GraphView {
        ...
        public void Init (Vector2 offset, float zoom) {
			...
			currentZoom = zoom;
			this.style.scale = new Scale(new Vector3 (zoom, zoom, zoom));
        ...
        private void _ViewTransformChanged (GraphView graphView) {
			...
			// If pan done.
			if ((Vector2)this.resolvedStyle.translate != contentOffset) {
				onPanDone?.Invoke (this.resolvedStyle.translate, contentOffset);
				//contentOffset = this.viewTransform.position;
				SetContentViewOffset (this.resolvedStyle.translate);
			}
        }
