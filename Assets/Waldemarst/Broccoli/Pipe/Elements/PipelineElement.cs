...
namespace Broccoli.Pipe {
	...
	public abstract class PipelineElement : ScriptableObject {
		...
		/// <param name="offset">Offset to apply to the seed.</param>
		/// </summary>
		public void PrepareSeed (int offset = 0) {
			if (isSeedFixed) {
				seed = fixedSeed + offset;
			} else if (pipeline != null) {
				seed = pipeline.seed + offset;
			} else {
				seed = (int)System.DateTime.Now.Ticks + offset;
			}
		...
		/// </summary>
		public void ResetFixedSeed (int offset = 0) {
			if (isSeedFixed) {
				PrepareSeed (offset);
			}
		}
