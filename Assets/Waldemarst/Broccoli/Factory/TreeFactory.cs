...
namespace Broccoli.Factory
{
	...
	public class TreeFactory : MonoBehaviour {
		...
		/// <summary>
		/// Delegate for Prefab paths.
		/// </summary>
		/// <param name="pipeline">Broccoli instance being processed.</param>
		/// <param name="path">Candidate path.</param>
		/// <returns>Path to use.</returns>
		public delegate string PrefabPathDelegate (
			Broccoli.Pipe.Pipeline pipeline, 
			string path);
		/// <summary>
		/// Delegate for Prefab paths.
		...
		public delegate string PrefabNameDelegate (
			Broccoli.Pipe.Pipeline pipeline, 
			string name);
		...
		/// <summary>
		/// Event called to override the Prefab path set.
		/// </summary>
		public PrefabPathDelegate OnSetPrefabPath;
		/// <summary>
		/// Event called to override the Prefab name set.
		/// </summary>
		public PrefabNameDelegate OnSetPrefabName;
		/// <summary>
		...
		/// <summary>
		/// Called after a LOD GameObject has been created, before adding it to the Prefab.
		/// </summary>
		...
		*/
		...
		public bool CreatePrefab () {
			...
			if (localPipeline != null && ValidatePipeline (localPipeline)) {
        		...
        		if (System.IO.Directory.Exists(System.IO.Path.GetDirectoryName(System.IO.Directory.GetParent(Application.dataPath) + "/" + treeFactoryPreferences.prefabSavePath))) {
					prefabSavePath = treeFactoryPreferences.prefabSavePath;
					if (!prefabSavePath.StartsWith ("Assets")) prefabSavePath = "Assets";
				} else {
					Debug.LogWarning ("Prefab destination path does not exists. Using Assets/ instead of " + treeFactoryPreferences.prefabSavePath);
					prefabSavePath = "Assets";
				}

				// Override Prefab Path.
				if (OnSetPrefabPath != null) prefabSavePath = OnSetPrefabPath.Invoke (localPipeline, prefabSavePath);
				...
				string prefabName = treeFactoryPreferences.prefabSavePrefix + prefabSuffix;

				// Override Prefab Name.
				if (OnSetPrefabName != null) prefabName = OnSetPrefabName.Invoke (localPipeline, prefabName);

				_assetManager.BeginWithCreatePrefab (previewTree.obj, prefabSavePath, prefabName);
				...
				pipelineElement = localPipeline.root;
				// Traverse Pipeline and get components, call their OnEndProcessPrefab method.
				do {
					if (pipelineElement.isActive && _componentManager.HasFactoryComponent(pipelineElement)) {
						_componentManager.GetFactoryComponent (pipelineElement).OnEndProcessPrefab (this);
					}
					pipelineElement = pipelineElement.sinkElement;
				} while (pipelineElement != null);
		...
		void OnAssetManagerLODReady (GameObject lodGO, int lodIndex) {
			if (onLODReady != null) onLODReady.Invoke (lodGO, lodIndex);
		}
		#endregion
