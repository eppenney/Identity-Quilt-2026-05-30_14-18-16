	...
	public class BakerComponent : TreeFactoryComponent {
			...
			TreeFactoryProcessControl ProcessControl = null) 
		{
			if (pipelineElement != null && tree != null) {
				...
				// AMBIENT OCCLUSION.
				bool enableAO = (ProcessControl.isPreviewProcess && bakerElement.enableAOInPreview) || (ProcessControl.isRuntimeProcess && bakerElement.enableAOAtRuntime) || ProcessControl.isPrefabProcess;
				if (enableAO) {
					...
					if (bakerElement.enableHierarchyAO) {
						treeFactory.meshManager.enableHierarchyAO = true;
						treeFactory.meshManager.hierarchyAOStrength = bakerElement.hierarchyAOStrength;
						treeFactory.meshManager.hierarchyAOCurve = bakerElement.hierarchyAOCurve;
					} else {
						treeFactory.meshManager.enableHierarchyAO = false;	
					}
				} else {
					treeFactory.meshManager.enableAO = false;
				}
				...
				if (bakerElement.addCollider) {
					AddCapsuleColliders (treeFactory);
				} else {
					RemoveCapsuleColliders ();
				}
				// MESH COLLIDERS.
				TrunkCustomMeshElement trunkCustomMeshElement = 
							(TrunkCustomMeshElement) bakerElement.GetUpstreamElement (PipelineElement.ClassType.TrunkCustomMesh);
				bool meshColliderAdded = false;
				if (trunkCustomMeshElement != null && trunkCustomMeshElement.isActive && trunkCustomMeshElement.ProcessingTrunkMeshDesc != null) {
					TrunkMeshDescriptor trunkDesc = trunkCustomMeshElement.ProcessingTrunkMeshDesc;
					if (trunkDesc != null && trunkDesc.hasCollider && trunkDesc.MeshCollider != null) {
						...
						AddMeshCollider (colliderMesh, treeFactory);
		...
		public override void OnProcessPrefab (TreeFactory treeFactory) {
			...
			treeFactory.meshManager.enableHierarchyAO = false;
			if (bakerElement.enableHierarchyAO) {
				treeFactory.assetManager.enableHierarchyAO = true;
				treeFactory.assetManager.hierarchyAOStrength = bakerElement.hierarchyAOStrength;
				treeFactory.assetManager.hierarchyAOCurve = bakerElement.hierarchyAOCurve;
			}
		...
		/// <summary>
		/// Adds a CapsuleCollider to each trunk.
		/// </summary>
		/// <param name="treeFactory">Tree factory.</param>
		protected void AddCapsuleColliders (TreeFactory treeFactory) {
			List<BroccoTree.Branch> rootBranches = tree.branches;
			Vector3 trunkBase;
			Vector3 trunkTip;
			RemoveCapsuleColliders ();

			// Check for custom trunk upstream, see if the selected custom trunk has an overriden capsule collider.
			TrunkCustomMeshElement trunkCustomMeshElement = 
				(TrunkCustomMeshElement) pipelineElement.GetUpstreamElement (PipelineElement.ClassType.TrunkCustomMesh);
			if (trunkCustomMeshElement != null &&
				trunkCustomMeshElement.ProcessingTrunkMeshDesc != null && 
				trunkCustomMeshElement.ProcessingTrunkMeshDesc.overridesCapsuleCollider)
			{
				Vector3 capsuleOrigin = Vector3.zero;
				float scale = treeFactory.treeFactoryPreferences.factoryScale;
				capsuleOrigin += trunkCustomMeshElement.ProcessingTrunkMeshDesc.capsuleColliderPosition * scale;
				capsuleOrigin.y += trunkCustomMeshElement.ProcessingTrunkMeshDesc.capsuleColliderHeight / 2f * scale;
				CapsuleCollider capsuleCollider = tree.obj.AddComponent<CapsuleCollider> ();
				capsuleCollider.radius = trunkCustomMeshElement.ProcessingTrunkMeshDesc.capsuleColliderRadius * scale;
				capsuleCollider.height = trunkCustomMeshElement.ProcessingTrunkMeshDesc.capsuleColliderHeight * scale;
				capsuleCollider.center = capsuleOrigin;
			} else {
				for (int i = 0; i < rootBranches.Count; i++) {
					...
					Vector3 basePos = trunkBase * scale;
					basePos.y = capsuleCollider.height * 0.5f;
					capsuleCollider.center = Vector3.Lerp(basePos, (trunkTip + trunkBase) / 2f * scale, bakerElement.colliderOriginToAvg);
				}
			}
		}
		/// <summary>
		/// Removes CapsuleCollider Components on the TreeFactory object.
		/// </summary>
		protected void RemoveCapsuleColliders () {
		...
		/// </summary>
		/// <param name="treeFactory">Tree factory.</param>
		protected void AddMeshCollider (Mesh colliderMesh, TreeFactory treeFactory) {
			RemoveMeshColliders ();
			MeshCollider meshCollider = tree.obj.AddComponent<MeshCollider> ();
			meshCollider.sharedMesh = colliderMesh;
			meshCollider.convex = false;
		}
		/// <summary>
		/// Removes MeshColliders Components on the TreeFactory object.
		/// </summary>
		protected void RemoveMeshColliders () {
