...
namespace Broccoli.Component
{
	...
	public class StructureGeneratorComponent : TreeFactoryComponent, IStructureGeneratorComponent {
		...
		void OnBeforeGenerateStructures (List<StructureGenerator.Structure> structures, StructureGenerator.StructureLevel structureLevel, StructureGenerator.Structure parentStructure, int structureIndex)
		{
			if (structureLevel.isSprout) {
				structureGeneratorElement.ResetFixedSeed (structureIndex);
			}
		}
		/// <summary>
		...
		private void StructureTree (TreeFactory treeFactory, bool useCache = false) {
			...
			if (structureGeneratorElement.trunkStructureLevel.trunkMode ==TrunkStructureLevel.TrunkMode.CustomMesh) {
				...
				if (trunkCustomMeshElement != null) {
					trunkCurves = trunkCustomMeshElement.GetTrunkBranchCurves (trunkCustomMeshElement.SelectedTrunkIndex);
				}
			}
