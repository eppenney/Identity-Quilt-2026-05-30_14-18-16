...
namespace Broccoli.Component
{
	...
	public class BranchMeshGeneratorComponent : TreeFactoryComponent {
			...
			TreeFactoryProcessControl processControl = null) 
		{
			...
			TrunkCustomMeshElement trunckCustomElement =
					(TrunkCustomMeshElement)branchMeshGeneratorElement.GetDownstreamElement (PipelineElement.ClassType.TrunkCustomMesh);
			if (trunckCustomElement != null) {
				trunckCustomElement.SetProcessingTrunkIndex (false);
				if (trunckCustomElement.SelectedTrunkIndex > -1) {
			...
			meshBuilder.segmentAngleOffset = branchMeshGeneratorElement.segmentAngleOffset * Mathf.Deg2Rad;
