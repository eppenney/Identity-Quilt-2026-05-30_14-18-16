...
namespace Broccoli.Factory
{
    ...
    public class SproutSubfactory {
        ...
        */
            ...
            foreach (BranchDescriptor.SproutStructure sproutStructure in snapshot.sproutStructures) {
                ...
                if (sproutStyle != null && sproutStructure.submeshCount > 0) {
                    for (int matI = sproutStructure.submeshIndex; matI < sproutStructure.submeshIndex + sproutStructure.submeshCount; matI++) {
                        ...
                        m.SetTexture ("_SproutTintDistribution", GetTintDistributionTex(sproutStructure.styleId, sproutStyle.sproutTintDistribution));
        ...
        Dictionary<Guid, Texture2D> _tintDistributionTexs = new Dictionary<Guid, Texture2D> ();
        private Texture2D GetTintDistributionTex(Guid styleId, AnimationCurve distributionCurve)
        {
            Texture2D distributionTex = null;
            if (_tintDistributionTexs.ContainsKey (styleId)) {
                distributionTex = _tintDistributionTexs[styleId];
            } else {
                distributionTex = new Texture2D(256, 1, TextureFormat.RFloat, false);
                distributionTex.wrapMode = TextureWrapMode.Clamp;
            }
            BakeCurveToTexture (distributionCurve, ref distributionTex);
            return distributionTex;
        }
        /// <summary>
        /// Bakes an AnimationCurve into a 2D texture (used as a 1D LUT) without generating garbage.
        /// It reuses the provided texture if it's valid; otherwise, it creates a new one.
        /// The curve's value is stored in the red channel.
        /// </summary>
        /// <param name="curve">The AnimationCurve to bake.</param>
        /// <param name="textureToUpdate">A reference to the Texture2D to update. If null or invalid, a new texture will be created and assigned back to this reference.</param>
        /// <param name="resolution">The width of the texture to create. Default is 256.</param>
        /// <returns>The updated or newly created Texture2D.</returns>
        private static Texture2D BakeCurveToTexture(AnimationCurve curve, ref Texture2D textureToUpdate, int resolution = 256)
        {
            if (curve == null)
            {
                Debug.LogError("Cannot bake a null curve.");
                if (textureToUpdate != null) UnityEngine.Object.DestroyImmediate(textureToUpdate);
                return null;
            }

            // 1. Check if the existing texture is valid. If not, create a new one.
            // We check the height to ensure it's a 1D-style texture.
            if (textureToUpdate == null || textureToUpdate.width != resolution || textureToUpdate.height != 1)
            {
                if (textureToUpdate != null)
                {
                    UnityEngine.Object.DestroyImmediate(textureToUpdate);
                }
                // Create a Texture2D with a height of 1 to act as a 1D texture.
                textureToUpdate = new Texture2D(resolution, 1, TextureFormat.RFloat, false);
                textureToUpdate.wrapMode = TextureWrapMode.Clamp;
            }

            // 2. Create a temporary array to hold the pixel data.
            Color[] pixels = new Color[resolution];

            // 3. Loop through, evaluate the curve, and store the value.
            ...
            // 4. Apply the pixel data to the existing texture and upload to the GPU.
            textureToUpdate.SetPixels(pixels);
            textureToUpdate.Apply(false); // 'false' makes it non-readable from CPU, which is more efficient if you don't need to read it back.
