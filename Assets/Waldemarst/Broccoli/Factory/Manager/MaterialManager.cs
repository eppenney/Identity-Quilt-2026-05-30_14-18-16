...
namespace Broccoli.Manager
{
	...
	public class MaterialManager {
			...
			if (preferredShader == TreeFactoryPreferences.PreferredShader.SpeedTree8 || preferredShader == TreeFactoryPreferences.PreferredShader.SpeedTree8Compatible) {
				...
				if (preferredShader == TreeFactoryPreferences.PreferredShader.SpeedTree8Compatible && customShader != null) {
					...
					if (leavesShaderPipelineType == RenderPipelineType.HDRP) {
						...
						if (leavesShader == null) leavesShader = Shader.Find("Universal Render Pipeline/Nature/SpeedTree8_PBRLit");
					} else {
			...
			if (preferredShader == TreeFactoryPreferences.PreferredShader.SpeedTree8 || preferredShader == TreeFactoryPreferences.PreferredShader.SpeedTree8Compatible) {
				...
				else
				{
					...
					else if (barkShaderPipelineType == RenderPipelineType.URP)
					{
						...
						if (barkShader == null) barkShader = Shader.Find("Universal Render Pipeline/Nature/SpeedTree8_PBRLit");
					}
					else
					{
		...
		public Material GetOwnedMaterial (int id, Shader shader) {
			if (shader == null)
            {
				Debug.LogWarning("MaterialManager: GetOwnedMaterial with null shader is not valid, cannot get a material.");
				return null;
            }
			Material material = null;
			// Get existing material to return if equals the shader.
			if (ownedMaterials.Contains (id) && materials.ContainsKey (id)) {
				if (materials [id].shader.name.Equals (shader.name)) {
					material = materials [id];
				} else {
					Object.DestroyImmediate (materials [id]);
					materials.Remove (id);
