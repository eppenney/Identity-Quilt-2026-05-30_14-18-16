...
namespace Broccoli.Manager
{
	...
	public class AssetManager {
		...
		/// </summary>
		/// <param name="lodGameObject">LOD Gameobject.</param>
		/// <param name="lodIndex">LOD index.</param>
		public delegate void OnLODEvent (GameObject lodGameObject, int lodIndex);
		/// <summary>
		/// To be called when a LOD GameObject is ready and before it gets added to the prefab.
		/// </summary>
		public OnLODEvent onLODReady;
		/// <summary>
		/// Delegate for material processing.
		/// </summary>
		/// <param name="material"></param>
		/// <param name="submeshIndex"></param>
		/// <returns></returns>
		public delegate Material OnMaterialDelegate(Material material, int submeshIndex);
		/// <summary>
		/// Used to override the material to save on the Prefab.
		/// </summary>
		public OnMaterialDelegate onAddMaterialToPrefab;
		#endregion
		...
		*/
		...
		public float hierarchyAOStrength = 0f;
		...
		public void BeginWithCreatePrefab (GameObject previewTarget, string prefabSavePath, string prefabName) {
			...
			Object[] allAssets = AssetDatabase.LoadAllAssetsAtPath (this.prefabFullPath);
		...
		Mesh GetMeshForPrefab (Matrix4x4 matrix, int lodIndex = 0) {
			...
			if (assetMeshes.ContainsKey (lodIndex)) {
				...
				if (enableAO || enableHierarchyAO) {
					...
					if (enableHierarchyAO) {
						List<Vector4> uv7 = new List<Vector4> ();
						mergingMesh.GetUVs (7, uv7);
						Broccoli.Utils.AmbientOcclusionBaker.BakeGirthAO (
							ref colors, 
							uv7.ToArray (), 
							hierarchyAOStrength,
							Broccoli.Factory.TreeFactory.GetActiveInstance ().previewTree.minGirth,
							Broccoli.Factory.TreeFactory.GetActiveInstance ().previewTree.maxGirth,
							hierarchyAOCurve,
							true);
					}
		...
		bool SavePrefab (bool generateBillboard, float billboardPercentage) {
			...
			if (prefabIsValid) {
				...
				while (materialsEnumerator.MoveNext ()) {
					...
					Material materialToAdd = materialPair.Value [0];
					
					bool overridesMaterial = false;
					if (onAddMaterialToPrefab != null) {
						Material overrideMaterial = onAddMaterialToPrefab.Invoke (materialToAdd, materialCount);
						if (overrideMaterial != materialToAdd) {
							materialToAdd = overrideMaterial;
							overridesMaterial = true;
						}
					}

					if (!overridesMaterial) {
						if (string.IsNullOrEmpty (AssetDatabase.GetAssetPath (materialToAdd))) {
							materialToAdd = Object.Instantiate<Material> (materialToAdd);
							materialToAdd.name = materialToAdd.name.Replace ("(Clone)", "");
							if (TreeFactory.GetActiveInstance ().treeFactoryPreferences.prefabIncludeMaterialsInsidePrefab) {
								AssetDatabase.AddObjectToAsset (materialToAdd, prefab);
							} else {
								string folderPath = GetPrefabResourceFolder ();
								string materialPath = folderPath + "/" + materialToAdd.name + ".mat";
								AssetDatabase.CreateAsset (materialToAdd, materialPath);
							}
						}
						if (materialParameters.ContainsKey (materialPair.Key)) {
							...
							case MaterialParams.ShaderType.Native:
								SetNativeMaterial (materialToAdd, false);
								break;
						...
						GetPrefabResourceFolder (); 
					}

					prefabMaterials.Add (materialToAdd);
					materialCount++;
				}
				...
				if (assetMeshes.Count == 1 && !generateBillboard) {
					...
					if (splitSubmeshesIntoGOs) {
						...
						if (onLODReady != null) onLODReady.Invoke (prefabGameObject, 0);
					} else {
						//Mesh lodMesh = CreateMeshGameObject (lodGameObject, i);
				...
				// LODs. 
					...
					for (i = 0; i < assetMeshes.Count; i++) {
						...
						if (splitSubmeshesIntoGOs) {
							...
							for (int submeshI = 0; submeshI < submeshCount; submeshI++) {
								...
								// Call onLODReady.
								if (onLODReady != null) onLODReady.Invoke (lodGameObject, i);
							}
					...
					if (generateBillboard) {
						...
						// Call OnLODReady
						if (onLODReady != null) onLODReady.Invoke (billboardGameObject, i);
		...
		public void AddMeshAsAsset (Mesh mesh, Object _prefab = null) {
			#if UNITY_EDITOR
			if (_prefab == null && this.prefab != null) _prefab = prefab;
			if (_prefab != null) {
				if (TreeFactory.GetActiveInstance ().treeFactoryPreferences.prefabIncludeMeshesInsidePrefab) {
		...
		public string GetPrefabResourceFolder () {
			return TextureManager.GetInstance ().GetOrCreateFolder (prefabSavePath, prefabName + "_Resources");
		}
		/// <summary>
