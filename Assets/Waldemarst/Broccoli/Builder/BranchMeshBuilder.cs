...
namespace Broccoli.Builder {
	...
	public class BranchMeshBuilder {
		...
		struct BranchJob : IJobParallelFor {
			...
			/// <summary>
			/// Adds offset to the radial position of segments.
			/// </summary>
			public float segmentAngleOffset;
			/// <summary>
				...
				if (polygonSides >= 3) {
					...
					for (int i = 0; i <= polygonSides; i++) {
						...
						if (shapeStartIndex >= 0) {
							...
							vertex = new Vector3 (
								Mathf.Cos (angle * i + segmentAngleOffset) * localGirth,
								Mathf.Sin (angle * i + segmentAngleOffset) * localGirth,
								0f);
		...
		/// <summary>
		/// Cancels the offset applied to the tree trunks.
		/// </summary>
		public bool cancelTrunkOriginOffsetEnabled = false;
		/// <summary>
		/// Radial offset to apply to the segments in the mesh.
		/// </summary>
		public float segmentAngleOffset = 0f;
		/// <summary>
		...
		public Mesh MeshTree (BroccoTree tree) { // TODO: cut
			...
			if (cancelTrunkOriginOffsetEnabled) {
		...
		public delegate void OnBranchSkinDelegate (BranchSkin branchSkin, BroccoTree.Branch firstBranch);
		public OnBranchSkinDelegate onCreateBranchSkin;
		/// <summary>
		...
		public BranchSkin GetOrCreateBranchSkin (BroccoTree.Branch firstBranch, int level, float lengthOffset, float hierarchyLevel) {
			...
			if (ContainsMeshBuilder (BuilderType.Welding)) {
				...
				weldingMeshBuilder.segmentAngleOffset = segmentAngleOffset;
			}
			do {
			...
			onCreateBranchSkin?.Invoke (branchSkin, firstBranch);
			return branchSkin;
		}
		/// <summary>
		...
		protected void PostprocessBranchSkins (
			Mesh treeMesh,
			List<Mesh> meshes,
			BranchSkin branchSkin, 
			BroccoTree.Branch firstBranch, 
			...
			BroccoTree.Branch parentBranch = null) 
		{
			...
			for (int i = 0; i < branchSkin.ranges.Count; i++) {
				IBranchMeshBuilder branchMeshBuilder = GetBuilder (branchSkin.ranges [i].builderType);
				if (branchMeshBuilder != null) {
					meshToAdd = branchMeshBuilder.PostprocessBranchSkinRange (treeMesh, i, branchSkin, firstBranch, parentBranchSkin, parentBranch);
					if (meshToAdd != null) {
		...
		protected Mesh MeshBranchSkinsJob () {
			...
			BranchJob _branchJob = new BranchJob () {
				...
				segmentAngleOffset = segmentAngleOffset,
