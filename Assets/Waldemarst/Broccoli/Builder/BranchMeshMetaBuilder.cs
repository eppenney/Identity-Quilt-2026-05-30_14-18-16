...
namespace Broccoli.Builder
{
	...
	public class BranchMeshMetaBuilder {
		...
		struct UVJob : IJobParallelFor {
			...
			public float mappingXOffset;
			public float mappingYOffset;
			/// <summary>
			...
			public void Execute(int i) {
				...
				if (uv5.x >= 0f) {
					...
					float uvU = uv8.x + mappingXOffset; 
					float uvV = uv8.y + mappingYOffset;
					if (applyLimits) {
			...
			float mappingHeight = 1f)
		{
			...
			uvJob.mappingXOffset = mappingXOffset;
			uvJob.mappingYOffset = mappingYOffset;
