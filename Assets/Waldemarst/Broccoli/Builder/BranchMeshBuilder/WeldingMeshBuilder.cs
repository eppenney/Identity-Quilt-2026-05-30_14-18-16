...
namespace Broccoli.Builder {
	...
	public class WeldingMeshBuilder : IBranchMeshBuilder {
		...
		/// <summary>
		/// Radial rotation to add to branch segment vertices (in radians).
		/// </summary>
			...
			if (polygonSides >= 3) {
				...
				for (int i = 0; i <= polygonSides; i++) {
					if (i != polygonSides) {
						...
						vertex = new Vector3 (
							Mathf.Cos ((angle * i) - weldingAngle + segmentAngleOffset),
							Mathf.Sin ((angle * i) - weldingAngle + segmentAngleOffset) * welding,
							0f);
						radialPosition = i * radialAngle; 
						// Welding angle rotation.
						// x' = x*cos(θ)-y*sin(θ)
