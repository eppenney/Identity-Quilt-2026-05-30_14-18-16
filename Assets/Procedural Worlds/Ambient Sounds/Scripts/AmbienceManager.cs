    ...
    public class AmbienceManager : MonoBehaviour
    {
        ...
        void OnEnable()
        {
            ...
            if (PreloadAudio)
            {
                ...
                foreach (AudioArea area in s_areaSequences)
                    if (area != null)
                        foreach (Sequence seq in area.m_activeSequences)
                            if (seq != null) {
                                foreach (Sequence.ClipData clip in seq.m_clipData)
                                    if(clip.m_clip != null)
                                        GetAudioData(clip.m_clip);
        ...
        private void Update()
        {
            ...
            for (int seq = 0; seq < cnt; ++seq)
            {
                ...
                if (fadeAmt > 0) {
                    for (int sid = 0; sid < ps.m_activeSequences.Length; ++sid)
                    {
                        Sequence data = ps.m_activeSequences[sid];
                        if (data == null)
                            continue;
                        bool found = false;
        ...
        void OnAudioSettingsChanged(bool deviceWasChanged)
        {
            ...
            foreach (AudioTrack track in s_sourceTrackData)
            {
                ...
                if (track.m_outputSource != null)
                    track.m_outputSource.clip = AudioClip.Create(track.m_name, lastSampleRate, Mathf.Clamp(track.outputChannelCount, 1, 8), lastSampleRate, true, track.OnAudioRead);
            }
            if (m_useAudioSource)
            {
                if (outputSourceGO)
        ...
        IEnumerator CoroCreateSource(AudioTrack track, int delay) {
            ...
            if (source == null)
                yield break;
            source.loop = true;
            source.clip = AudioClip.Create(track.m_name, lastSampleRate, Mathf.Clamp(track.outputChannelCount, 1, 8), lastSampleRate, true, track.OnAudioRead);
            source.Play();
        }
        ...
        //Helper to deal with the FindObjectOfType API changes
        new T FindObjectOfType<T>() where T : UnityEngine.Object
        {
        ...
        //Helper to deal with the FindObjectsOfType API changes
        new T[] FindObjectsOfType<T>() where T : UnityEngine.Object
        {
#if UNITY_6000_0_OR_NEWER
        ...
        public static List<TrackPlayingInfo> GetTracks()
        {
            ...
            for (int t = 0; t < cnt; ++t)
            {
                ...
                {
                    m_name = track.IsDelaying ? "Delaying  [" + track.CurProgress.ToString("N2") + ":" + track.m_sequence.TrackDelayTime.ToString("N2") + "]" : (curPlaying == null ? "NULL" : curPlaying.Name + " - " + track.CurPlaybackSpeed.ToString("N2") + "x [" + track.CurProgress.ToString("N2") + ":" + curPlaying.Length.ToString("N2") + "]"),
        ...
        public static bool IsSequenceActive(Sequence seq)
        {
            ...
            for (int a = 0; a < numAreas; ++a)
            {
                ...
                int numSeq = aa.m_activeSequences.Length;
                for (int s = 0; s < numSeq; ++s)
                {
                    Sequence ss = aa.m_activeSequences[s];
                    if (ss == seq)
                        return true;
                }
            }
