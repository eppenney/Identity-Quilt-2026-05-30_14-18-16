...
namespace AmbientSounds {
    ...
    internal class AudioTrack {
        ...
        /// <summary> Number of loops this track has completed if sequence max loop count > 0 </summary>
        ...
        private int NextClipIdx {
            get {
                if (_nextClipIdx < 0) {
                    if (m_sequence.m_loopCount > 0 && curLoopCount >= m_sequence.m_loopCount)
                    {
                        LoopsExpired = true;
                        m_sequence.TrackDelayTime = float.PositiveInfinity;
                        return -1;
                    }
                    int numClips;
                    if (m_isOutputDirect)
                    ...
                    if (numClips > 0) {
                        if (++trackQueuePos >= numClips || trackQueue == null || trackQueuePos >= trackQueue.Length)
                        {
                            //we hit the end so we need to reset queue
                            if (m_sequence.m_loopCount > 0)
                                ++curLoopCount;
                            else
                                curLoopCount = 0;
        ...
        /// <summary> True if waiting for Clip's TrackDelayTime before playing next track </summary>
        public bool LoopsExpired { get; private set; }
        ...
        public AudioTrack(Sequence source, bool playOnce = false) {
            ...
            NextClipIdx = -1;
            if (m_sequence.m_clipData.Length > currentClipIdx && currentClipIdx >= 0)
        ...
        public void UpdateDirect() {
            ...
            if (m_outputSource.clip == null) {
                ...
                currentClipIdx = NextClipIdx;
                NextClipIdx = -1;
                m_outputSource.clip = currentClipIdx < 0 ? null : DirectClipData[currentClipIdx];
            ...
            if (DirectFadeOut) { //Clips list changed and we need to fade the current clip then change it.
                ...
                if (CurFade <= 0) {
                    ...
                    currentClipIdx = NextClipIdx;
                    NextClipIdx = -1;
                    m_outputSource.clip = currentClipIdx < 0 ? null : m_sequence.Clips[currentClipIdx].m_clip;
                    m_outputSource.Play();
            ...
            if (m_sequence != null)
            {
                if (IsDelaying)
                {
                    ...
                    {
                        if (LoopsExpired && (m_sequence.m_loopCount <= 0 || curLoopCount < m_sequence.m_loopCount))
                        {
                            LoopsExpired = false;
                            m_sequence.TrackDelayTime = 1;
                        }
        ...
        public void GetFrame(double step, float volume, float[] ret, int startOffset, int numChannels) {
            ...
            if (m_sequence != null)
            {
                if (IsDelaying)
                {
                    if (trackTime >= m_sequence.TrackDelayTime)
                    {
                        //Debug.Log("Ending delay of " + m_sequence.TrackDelayTime + " seconds at time " + trackTime);
                ...
                else if (LoopsExpired)
                {
                    return;
                }
            }
            //check for end of track and skip to next if needed
            if (trackTime >= currentClipData.Length) {
                if (currentClipIdx >= 0 && currentClipIdx < m_sequence.Clips.Length)
                    lock(m_sequence.ClipsLockHandle)
                ...
                if (m_sequence != null) {
                    if (!isSynced && m_sequence.TrackDelayTime > 0f) {
                        ...
                        m_outputNeedsMoved = true;
                        currentClipIdx = NextClipIdx;
                        //Debug.Log("Starting delay of " + m_sequence.TrackDelayTime + " seconds. Switching to clip " + currentClipIdx + " at time " + trackTime + " with clip length=" + currentClipData.Length);
                        ...
                        m_outputNeedsMoved = true;
                        currentClipIdx = NextClipIdx;
                        float crossFadeValue = Mathf.Min(m_sequence.m_crossFade, Mathf.Min(currentClipData.Length / 2.0f, RawClipData[currentClipIdx].Length / 2.0f));
                        //Debug.Log("Switching to clip " + currentClipIdx + " at time " + trackTime + " with crossfade of " + crossFadeValue);
                        ...
                        if (!startedCrossFade)
                            lock(m_sequence.ClipsLockHandle)
                                StartedPlaying = currentClipIdx < 0 ? null : m_sequence.Clips[currentClipIdx].m_clip;
                        else
                            startedCrossFade = false;
                    }
