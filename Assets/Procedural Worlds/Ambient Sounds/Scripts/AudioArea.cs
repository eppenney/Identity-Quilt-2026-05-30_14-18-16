...
namespace AmbientSounds {
    ...
    public class AudioArea : MonoBehaviour {
        ...
        [NonSerialized]
        public Sequence[] m_activeSequences = null;
        ...
        {
            m_activeSequences = new Sequence[m_sequences.Length];
            for (int s = 0; s < m_sequences.Length; ++s)
            {
                if (m_sequences[s] != null && m_sequences[s].m_allowMultipleInstances)
                    m_activeSequences[s] = Instantiate(m_sequences[s]);
                else
                    m_activeSequences[s] = m_sequences[s];
            }
        }
        #endregion
