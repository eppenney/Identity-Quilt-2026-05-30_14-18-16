...
namespace AmbientSounds {
    ...
    public class SequenceDataEditor : PWEditor, IPWEditor {
        ...
        SerializedProperty m_loopCount;
        SerializedProperty m_allowMultipleInstances;
        ...
        void OnEnable() {
            ...
            m_allowMultipleInstances = serializedObject.FindProperty("m_allowMultipleInstances");
        ...
        public void ClipsPanel(bool inlineHelp) {
            ...
            EditorGUI.BeginChangeCheck();
            m_editorUtils.PropertyField("m_allowMultipleInstances", m_allowMultipleInstances, inlineHelp);
            if (EditorGUI.EndChangeCheck() && Application.isPlaying && target is Sequence seq)
            {
                AudioArea[] AllAreas = Resources.FindObjectsOfTypeAll<AudioArea>();
                foreach (AudioArea area in AllAreas)
                    if (System.Array.IndexOf(area.m_sequences, seq) >= 0)
                        area.UpdateSequences();
            }
            m_editorUtils.PropertyField("mLoopCount", m_loopCount, inlineHelp);
